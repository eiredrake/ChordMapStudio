...
let audible=false,match=false,audioGesture=false;
class AudioContext extends Element {
  constructor(){super();this.state=audioGesture?'running':'suspended';this.sampleRate=48000;}
  async resume(){if(audioGesture){this.state='running';await this.fire('statechange');}} async close(){this.state='closed';}
...
vm.runInContext(fs.readFileSync(path.join(root,'tuner.js'),'utf8'),context);
...
(async()=>{
  ...
  await flush();assert.equal(el('practice-start').disabled,true,'Autoplay-blocked audio waits for interaction');
  audioGesture=true;await document.fire('pointerdown');advance(1600);
  ...
  assert.match(el('practice-live-notes').textContent,/Heard notes: E/);
  assert.match(el('practice-live-detail').textContent,/Missing or weak: C \/ G/);
  advance(940);
  ...
  devices=devices.filter(d=>d.deviceId!=='mic1');await tracks[0].fire('ended');await media.fire('devicechange');assert.equal(tracks[0].readyState,'ended');assert.match(el('listen-status').textContent,/disconnected/);
  el('practice-source').value='usb';await el('practice-source').fire('change');await flush();
  assert.equal(captureCalls.at(-1).audio.deviceId.exact,'usb1');advance(1600);
  ...
  assert.match(el('tuner-target').textContent,/Low E.*82.41 Hz/);
  assert.equal(el('practice-device').value,'usb1');
  const activeTrack=tracks.at(-1);
  context.GuitarTuner.pitch=()=>({frequency:110,confidence:.99});
  audible=true;
  for(const [i,midi] of [40,45,50,55,59,64].entries()) {
    context.GuitarTuner.pitch=()=>({frequency:440*2**((midi-69)/12),confidence:.99});
    advance(400);assert.match(el('tuner-progress').textContent,new RegExp('String '+(i+1)+' of 6'),'A brief match does not advance');
    advance(800);
  }
  assert.match(el('tuner-progress').textContent,/All six strings tuned/);
  advance(2000);assert.match(el('tuner-progress').textContent,/All six strings tuned/);
  assert.equal((el('tuner-neck').innerHTML.match(/tuner-string-done/g)||[]).length,6);
  await el('tuner-restart').fire('click');assert.match(el('tuner-target').textContent,/Low E/);
  context.GuitarTuner.pitch=()=>({frequency:82.4069,confidence:.99});advance(600);
  audible=false;advance(1800);audible=true;advance(600);
  assert.match(el('tuner-target').textContent,/Low E/,'Silence resets confirmation');
  advance(600);assert.match(el('tuner-target').textContent,/A \(5\)/);
  advance(1500);assert.match(el('tuner-target').textContent,/A \(5\)/,'Ringing prior string cannot advance next target');
  context.GuitarTuner.pitch=()=>({frequency:110,confidence:.99});
  audible=true;advance(240);
  assert.equal(el('tuner-note').textContent,'A2');assert.match(el('tuner-direction').textContent,/In tune/);
  context.GuitarTuner.pitch=()=>({frequency:108,confidence:.99});advance(240);
  assert.match(el('tuner-direction').textContent,/tune up/);
  context.GuitarTuner.pitch=()=>({frequency:112,confidence:.99});advance(240);
  assert.match(el('tuner-direction').textContent,/tune down/);
  audible=false;advance(160);assert.equal(el('tuner-note').textContent,'—','Silence clears stale pitch');
  await el('tuner-modal').fire('cancel',{preventDefault(){}});
  assert.equal(el('tuner-modal').open,false);assert.equal(activeTrack.readyState,'live');
  await el('tuner-open').fire('click');
  const tunerBackdrop={target:el('tuner-modal'),clientX:10,clientY:10};
  await el('tuner-modal').fire('pointerdown',tunerBackdrop);await el('tuner-modal').fire('click',tunerBackdrop);
  assert.equal(el('tuner-modal').open,false);
  devices.push({kind:'audioinput',deviceId:'mic2',label:'Other microphone'});
  const beforeRefresh=captureCalls.length,previousTrack=tracks.at(-1);
  await el('listen-refresh').fire('click');
  assert.equal(captureCalls.length,beforeRefresh);assert.equal(previousTrack.readyState,'live');
  el('practice-source').value='mic';await el('practice-source').fire('change');
  el('practice-device').value='mic2';await el('practice-device').fire('change');
  assert.equal(previousTrack.readyState,'ended');assert.equal(captureCalls.at(-1).audio.deviceId.exact,'mic2');
  advance(1600);
  const callsBefore=captureCalls.length;
  await el('tuner-open').fire('click');await flush();advance(1600);
  assert.equal(captureCalls.length,callsBefore,'Opening tuner reuses the active input');
  await el('listen-refresh').fire('click');assert.equal(captureCalls.length,callsBefore,'Refresh does not restart capture');
  assert.equal(tracks.at(-1).readyState,'live');
  await el('tuner-close').fire('click');
  await context.window.fire('pagehide');assert.equal(tracks.at(-1).readyState,'ended');
  const original=media.getUserMedia;media.getUserMedia=async()=>{throw Object.assign(new Error(),{name:'NotAllowedError'});};
  await el('practice-device').fire('change');assert.match(el('listen-status').textContent,/permission was denied/);media.getUserMedia=original;
  let resolve;pendingCapture=new Promise(r=>resolve=r);
  const waiting=el('practice-device').fire('change');await flush();await context.window.fire('pagehide');
